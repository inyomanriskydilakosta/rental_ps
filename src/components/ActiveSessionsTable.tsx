              ...
              <tr key={session.id} className="hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-4 py-3.5">
                  <button
                    ...
                    disabled={isPending && endingId === session.id}
      ...
      <div className="sm:hidden divide-y divide-gray-50">
        {paginated.length === 0 ? (
          ...
          <div key={session.id} className="p-4 hover:bg-gray-50/40 transition-colors">
            ...
            {/* Row bottom: type + time + end button */}
            <div className="flex items-center gap-3 pl-7">
              ...
              <button
                onClick={() => handleEndSession(session.id)}
                disabled={isPending && endingId === session.id}
                className="ml-auto inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 transition-colors disabled:opacity-60 flex-shrink-0"
              >
                {isPending && endingId === session.id
                  ? <Loader2 className="w-3 h-3 animate-spin" />
                  : <CheckCircle className="w-3 h-3" />}

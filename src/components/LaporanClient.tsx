...
import { Download, Search, Clock, DollarSign, CheckCircle2, AlertCircle, Calendar, Loader2 } from 'lucide-react';
import { usePagination } from '@/hooks/usePagination';
import Pagination from '@/components/Pagination';
import { updateTransactionStatus } from '@/app/laporan/actions';
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
        ...
        <div className="hidden sm:block overflow-x-auto">
          <table className="w-full">
            <thead><tr className="bg-gray-50/80">{['No', 'Nama Pelanggan', 'Unit PS', 'Waktu', 'Durasi', 'Total', 'Status', 'Tanggal'].map((h) => <th key={h} className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">{h}</th>)}</tr></thead>
            <tbody className="divide-y divide-gray-50">
              {paginated.length === 0 ? (
                <tr><td colSpan={8} className="px-5 py-10 text-center text-sm text-gray-400">Belum ada transaksi.</td></tr>
              ) : paginated.map((t, i) => (
              ...
              <div key={t.id} className="p-4 hover:bg-gray-50/40 transition-colors">
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <button
                    onClick={() => handleToggleStatus(t)}
                    disabled={isPending && updatingId === t.id}
                    className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold flex-shrink-0 border transition-all hover:opacity-80 ${t.status === 'LUNAS' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-600 border-red-200'}`}
                  >
                    {isPending && updatingId === t.id ? <Loader2 className="w-3 h-3 animate-spin" /> : t.status === 'LUNAS' ? <CheckCircle2 className="w-3 h-3" /> : <AlertCircle className="w-3 h-3" />}
